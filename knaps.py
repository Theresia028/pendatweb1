...
with preporcessing:
    ...
    df[["Age", "Gender", "Family_Diabetes", "highBP", "PhysicallyActive", "BMI", "Smoking", "Alcohol", "Sleep","SoundSleep","RegularMedicine","JunkFood","Stress","BPLevel","Pregancies","Pdiabetes","UriationFreq","Diabetic"]].agg(['min','max'])
    ...
    labels = pd.get_dummies(df.diabate).columns.values.tolist()
    ...
    dataubah=df.drop(columns=['Age','gender','Family_Diabetes','highBP','Smoking','Alcohol','Sleep','SoundSleep','RegularMedicine','Stress','BPLevel','Pregancies','Pdiabetes','UriationFreq','Diabetic'])
    dataubah
